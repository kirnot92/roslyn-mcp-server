...
ROOT = project_root()
REPO_ROOT = repo_root("ROSLYN_MCP_POWERSHELL_ROOT", "PowerShell")
STAMP = os.environ.get("POWERSHELL_GOTODEF_RAMP_STAMP") or datetime.now().strftime("%Y%m%d-%H%M%S")
LOG_FILE = local_path(f"powershell-gotodef-ramp-{STAMP}.log")
STDERR_FILE = local_path(f"powershell-gotodef-ramp-{STAMP}-stderr.log")
RAW_FILE = local_path(f"powershell-gotodef-ramp-{STAMP}-raw.json")
CHECKPOINTS = [
    int(value)
    for value in os.environ.get("POWERSHELL_GOTODEF_CHECKPOINTS", "0,5,10,15,20,30,45,60,90,120,180,240").split(",")
    if value.strip()
]

SOURCE_FILE = r"src\System.Management.Automation\engine\CommandDiscovery.cs"
PROBES = [
    {
        "name": "ExecutionContext",
        "line": 33,
        "expectedFiles": ["src/System.Management.Automation/engine/ExecutionContext.cs"],
    },
    {
        "name": "CommandInfo",
        "line": 60,
        "expectedFiles": ["src/System.Management.Automation/engine/CommandInfo.cs"],
    },
    {
        "name": "ScriptBlock",
        "line": 66,
    ...
    },
    {
        "name": "FunctionInfo",
        "line": 79,
        "expectedFiles": ["src/System.Management.Automation/engine/FunctionInfo.cs"],
    },
    {
        "name": "OutLineOutputCommand",
        "line": 160,
        "expectedFiles": ["src/System.Management.Automation/FormatAndOutput/out-textInterface/OutTextInterface.cs"],
    },
    {
        "name": "FormatDefaultCommand",
        "line": 160,
        "expectedFiles": ["src/System.Management.Automation/FormatAndOutput/format-default/format-default.cs"],
    },
    {
        "name": "CmdletInfo",
        "line": 163,
        "expectedFiles": ["src/System.Management.Automation/engine/CmdletInfo.cs"],
    },
    {
        "name": "SessionStateCmdletEntry",
        "line": 163,
        "expectedFiles": ["src/System.Management.Automation/engine/InitialSessionState.cs"],
    },
    {
        "name": "AliasInfo",
        "line": 178,
        "expectedFiles": ["src/System.Management.Automation/engine/AliasInfo.cs"],
    },
    {
        "name": "SessionStateAliasEntry",
        "line": 178,
        "expectedFiles": ["src/System.Management.Automation/engine/InitialSessionState.cs"],
    },
]
...
class McpClient:
    def __init__(self):
        ...
        install_dir = os.path.join(os.environ["LOCALAPPDATA"], "Microsoft", "dotnet")
        env = dict(os.environ)
        env["DOTNET_ROOT"] = install_dir
        env["PATH"] = install_dir + os.pathsep + os.path.join(os.environ["USERPROFILE"], ".dotnet", "tools") + os.pathsep + env["PATH"]
        self.next_id = 1
        self.responses = {}
...
def map_definition_result(probe, result, status, elapsed, checkpoint, started, completed):
    ...
    expected_files = [normalize_path(path) for path in probe["expectedFiles"]]
    matched = [
        item
        for item in items
        if normalize_path(item.get("file")) in expected_files
    ]
    return {
    ...
    try:
        raw["initialize"] = client.request(
            "initialize",
            {
                ...
                "capabilities": {},
                "clientInfo": {"name": "roslyn-mcp-powershell-gotodef-ramp", "version": "0.1"},
            },
            timeout=60,
        )
        ...
        for name, args, timeout in [
            ("list_workspaces", None, 45),
            ("load_solution", {"path": "PowerShell.sln"}, 120),
        ]:
