...
LOG_FILE = local_path("aspnetcore-smoke.log")
RAW_FILE = local_path("aspnetcore-smoke-raw.json")
WARMUP_SECONDS = int(os.environ.get("ASPNETCORE_SMOKE_WARMUP_SECONDS", "0"))
    ...
    try:
        raw["initialize"] = client.request(
            "initialize",
            {
                ...
                "capabilities": {},
                "clientInfo": {"name": "roslyn-mcp-aspnetcore-smoke", "version": "0.1"},
            },
            timeout=60,
        )
        ...
        if WARMUP_SECONDS > 0:
            ...
            rows.append(call_tool(client, "get_workspace_status", timeout=30, note=f"poll +{WARMUP_SECONDS}s"))
        else:
            time.sleep(3)
        ...
        rows.append(call_tool(client, "hover", {"file": file, "line": 17, "column": 25}, timeout=45, note="HttpContext class"))
        rows.append(call_tool(client, "go_to_definition", {"file": file, "line": 27, "column": 28}, timeout=60, note="HttpRequest"))
        rows.append(call_tool(client, "find_references", {"file": file, "line": 17, "column": 25, "includeDeclaration": True, "maxResults": 20}, timeout=60, note="HttpContext class"))
        rows.append(call_tool(client, "find_symbols", {"query": "HttpContext", "maxResults": 300}, timeout=60))
