...
Tier 1 실제 대형 repository opt-in 테스트를 본격적으로 실행한다. Startup initial solution load와 diagnostics notification offload는 구현되어 있으므로, 이 단계에서는 기본 timeout, result limit, warming metadata, diagnostics queue/cache 정책을 실제 결과에 맞춰 조정한다.
...
- `solution/open`/`project/open` 명시 로드가 Tier 1 실제 repository에서도 작은 fixture와 같은 의미 있는 symbol index를 만드는지 확인 필요
