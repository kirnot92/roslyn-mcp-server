...
- `find_symbols`
  ...
  - 후속 개선으로 `kindFilter`를 추가할 수 있다. 이 필터는 Roslyn LS의 `workspace/symbol` 검색 비용을 직접 줄이는 계약이 아니라, MCP 반환 결과를 LSP `SymbolKind` 기준으로 제한해 agent 탐색 노이즈를 줄이는 목적이다.

후속 read productivity 후보:
