...
src/
  RoslynMcpServer/
    ...
    Mcp/
      WorkspaceTools.cs
      NavigationTools.cs        # M2+
      DiagnosticsTools.cs       # M2+
      ToolModels.cs
    Workspace/
      WorkspaceScanner.cs
      WorkspaceSession.cs
    ...
    Lsp/
      ...
      LspMessages.cs
      LspModels.cs
      DiagnosticStore.cs        # M2+
    Infrastructure/
      Clock.cs
      UserFacingException.cs
tests/
  RoslynMcpServer.Tests/
  RoslynMcpServer.IntegrationTests/
    Fixtures/
      SampleSolution/
```
...
// M2+: DocumentStateManager, DiagnosticStore, navigation/diagnostics tools.
...
`get_workspace_status`는 M1에서 repository root, 후보 목록, 현재 target, load state, Roslyn LS process 상태, pending request 수를 반환한다. M2+에서는 최근 diagnostics 수신 시각과 열린 문서 수도 추가할 수 있다. `diagnostics` 같은 tool은 필요한 범위만 기다리고, workspace 전체가 안정될 때까지 무기한 대기하지 않는다.
...
대규모 repo에서 `workspace/symbol`, `textDocument/references`, diagnostics 관련 응답은 커질 수 있다. 모든 LSP request wrapper는 다음 옵션을 받는다.

```csharp
public sealed record LspRequestOptions(
    TimeSpan Timeout,
    int? MaxResults,
...
LSP 자체가 `MaxResults`를 지원하지 않는 method도 있으므로, 지원하지 않는 경우에는 응답 mapper에서 결과를 잘라내고 `truncated: true`를 tool 결과에 포함한다.
...
- `LspClient` write는 `_writeLock`으로 한 번에 하나만 쓴다.
...
`tests/RoslynMcpServer.IntegrationTests/Fixtures/SampleSolution`을 둔다.
...
1. `document_symbols`, `hover`, `go_to_definition`, `find_references`, `diagnostics`를 호출한다.
...
첫 구현 세션의 범위는 M0/M1로 제한한다. navigation tool과 diagnostics tool은 후속 구현으로 남긴다.
