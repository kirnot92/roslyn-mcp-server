...
- `find_implementations`도 위치 기반 tool이다. LSP `textDocument/implementation`을 호출하고, `find_references`와 같은 bounded location metadata를 반환해야 한다.
...
- `find_implementations` smoke는 interface/type/member 위치에서 구현 위치가 반환되는지 확인하고, warming 중 partial metadata를 허용한다.
