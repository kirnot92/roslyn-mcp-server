...
- 현재 read-only Roslyn tool은 `document_symbols`, `hover`, `go_to_definition`, `peek_definition`, `find_references`, `peek_references`, `find_implementations`, `find_symbols`, `diagnostics`다.
...
- 다음 read productivity 후보는 `get_call_hierarchy`, `get_type_hierarchy`, `get_completions`다. `get_call_hierarchy`는 callable의 incoming/outgoing 호출 관계를 다루고, `get_type_hierarchy`는 타입의 base/derived 관계를 다루므로 역할을 분리한다.
...
- `peek_references` smoke는 reference 위치와 snippet이 root-relative path, 1-based range, bounded text로 반환되는지 확인한다.
