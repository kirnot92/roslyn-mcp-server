...
- `LspReady`/`WorkspaceWarming`에서는 best-effort 실행과 `workspaceState`, `completeness` metadata를 포함해야 한다.
...
- load 후 상태가 `LspReady`, `WorkspaceWarming`, `Ready`, `Failed` 중 명확히 드러나는지 본다.
- read tool이 partial/unknown metadata와 함께 응답하는지 본다.
- 대량 결과가 제한되는지 본다.
- 결과를 `docs/smoke-tests/*.md`에 기록하게 한다.

임시 client script, log, cloned repo, `.local/` 파일은 커밋하지 않는다.
