...
- `find_symbols`
  - 이름 또는 패턴으로 심볼 검색
  - 현재는 전체 workspace symbol 결과를 반환하며, 심볼 종류별 필터링은 후속 개선으로 남긴다.
...
- `find_symbols` kind filtering
  - 기존 `find_symbols`에 `kindFilter` 옵션을 추가해 `class`, `interface`, `method`, `property`, `field` 등 LSP `SymbolKind` 기준으로 결과를 제한한다.
  - LSP `workspace/symbol` 요청 자체는 query 중심이므로 Roslyn LS 검색 비용을 직접 줄인다고 보장하지 않는다. MCP 반환 결과와 agent 노이즈를 줄이는 목적의 client-side filter로 둔다.
  - `totalKnown`, `returned`, `truncated` metadata는 필터 적용 후 기준과 원 응답 기준이 혼동되지 않도록 명확히 정의한다.
...
- `find_symbols` kind filtering
  ...
  - metadata는 원 응답 수와 필터 후 반환 수의 의미가 혼동되지 않도록 설계한다.
- `get_call_hierarchy`
