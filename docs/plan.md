...
내부에서는 `roslyn-language-server --stdio --autoLoadProjects`를 자식 프로세스로 띄우고, 필요한 LSP 초기화 및 요청/응답 관리를 담당한다.
...
첫 구현 세션은 M0/M1까지만 진행한다. 이 범위에서 구현할 MCP tool은 workspace 선택과 상태 확인에 필요한 도구로 제한한다.
...
M2부터 Agent CLI에서 실제로 유용하고 구현 난도가 낮은 읽기 중심 기능을 추가한다.
...
첫 구현 세션은 M0/M1까지만 진행한다. Target framework는 `net10.0`으로 시작한다. `load_solution`/`load_project`를 완성하기 전에 `roslyn-language-server`를 실제로 실행해 stdio LSP initialize와 workspace 로드 방식을 확인하고, 그 결과를 구현에 반영한다.
...
- MCP C# SDK 연결
- stdio MCP 서버 hello tool 구현
- `README.md`는 짧은 영어 소개로 유지

### M1: Roslyn LS 프로세스 연결

- `roslyn-language-server --stdio --autoLoadProjects` 실행
