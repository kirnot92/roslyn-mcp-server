...
- `peek_references`
  - 파일, 라인, 컬럼 기준 reference 위치를 찾고 각 위치 주변 source snippet을 함께 반환한다.
  - LSP `textDocument/references` 흐름을 사용한다.
  - 입력은 optional `includeDeclaration`, optional `maxResults`, optional `contextLines`를 둔다.
  - snippet은 `peek_definition`과 같은 path guard, document size, line/context, character cap 제한을 따른다.
  - 대형 repo에서 결과와 snippet 읽기 비용이 커질 수 있으므로 expensive request limit, result cap, snippet cap을 적용한다.

- `get_call_hierarchy`
  - 파일, 라인, 컬럼 기준으로 특정 메서드/생성자/속성 접근자의 incoming 또는 outgoing call 관계를 반환한다.
  - LSP call hierarchy의 `textDocument/prepareCallHierarchy`, `callHierarchy/incomingCalls`, `callHierarchy/outgoingCalls` 흐름을 사용한다.
  - 입력은 `direction`, optional `maxDepth`, optional `maxResults`를 둔다. 초기 구현은 대형 repo 폭주를 피하기 위해 `maxDepth = 1` 중심으로 제한한다.
  - `get_type_hierarchy`와 역할이 다르다. `get_type_hierarchy`는 base/derived type 같은 타입 상속 관계를 보고, `get_call_hierarchy`는 특정 callable symbol의 호출자/피호출자 관계를 본다.
...
- `get_type_hierarchy`
  ...
  - 호출 흐름 분석에는 사용하지 않는다. 호출 관계는 `get_call_hierarchy`가 담당한다.
...
- `find_symbols` kind filtering - 완료
  - 입력: `query`, optional `maxResults`, optional `kindFilter`
  - 기존 LSP `workspace/symbol` 결과를 `class`, `interface`, `method`, `property`, `field` 등 LSP `SymbolKind` 기준으로 제한한다.
...
- `get_call_hierarchy`
  ...
  - 대형 solution에서 호출 그래프 폭주를 막기 위해 expensive request limit, depth cap, result cap을 적용한다.
