...
- `find_symbols` kind/match filtering - 완료
  - 입력: `query`, optional `maxResults`, optional `kindFilter`, optional `matchMode`
  - 기존 LSP `workspace/symbol` 결과를 `class`, `interface`, `method`, `property`, `field` 등 LSP `SymbolKind`와 `default`, `exact`, `prefix`, `contains` symbol name match mode 기준으로 제한한다.
  - Roslyn LS 검색 비용 절감보다는 MCP 반환 결과의 noise 감소와 agent 후속 탐색 효율 개선을 목표로 한다.
  - metadata는 필터 후 mappable 결과 기준의 `totalKnown`, `returned`, `truncated`와 필터 전 mappable 결과 수인 `totalUnfilteredKnown`을 구분한다.
- `peek_references` - 완료
