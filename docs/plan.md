...
제품 포지션은 best-effort read-only Roslyn context provider로 고정한다. 이 서버는 대형 repository에서 Agent CLI가 C# 코드를 읽고, 검토하고, 탐색하는 데 필요한 compiler-backed context를 제공한다. 파일 변경은 agent의 일반 workspace editing 흐름이 담당하며, rename/code action/formatting/apply 같은 write/refactoring tool은 이 프로젝트의 방향에 포함하지 않는다.
...
2026-05-18 현재 `main` 기준으로 M0/M1 workspace tool, M2 read-only tool, M3 사용자/클라이언트 사용성 정리, M4 일부 품질 강화, M5 read productivity tool 일부가 완료되어 있다. 현재 제공하는 workspace tool은 다음과 같다.
...
- `find_symbols`
  - 이름 또는 패턴으로 심볼 검색
  - optional `kindFilter`로 `class`, `interface`, `method`, `property`, `field` 같은 LSP `SymbolKind` 이름 기준의 MCP-side 필터링을 지원한다.
  - optional `matchMode`로 `default`, `exact`, `prefix`, `contains` 기준의 simple symbol name 필터링을 지원한다.
  - optional `includePathPrefixes`로 root-relative path prefix 아래에 있는 symbol location만 유지한다.
  - 필터는 Roslyn LS `workspace/symbol` 응답 이후 적용되므로 Roslyn LS 검색 비용 절감을 보장하지 않는다.
...
- `peek_references` - 완료
  - reference 위치와 bounded source snippet 반환

- `find_implementations`
  - 파일, 라인, 컬럼 기준 interface, abstract member, type 구현 위치 반환

- `get_call_hierarchy`
  - 파일, 라인, 컬럼 기준 특정 메서드/생성자/속성 접근자의 직접 depth-1 incoming 또는 outgoing call 관계 반환

- `get_type_hierarchy`
  - 파일, 라인, 컬럼 기준 type identifier에서 base type, derived type, interface implementation 계층 반환
  - `direction`, `maxDepth`, `maxResults`로 방향과 bounded BFS 범위를 제한
...
`get_completions`는 파일, 라인, 컬럼 기준으로 IDE 자동완성 후보를 조회하는 기능이라 서버가 직접 파일을 수정하지 않더라도 주 용도가 코드 작성 보조에 가깝다. 이 프로젝트의 제품 방향은 best-effort read-only Roslyn context provider이므로, `get_completions`는 현재 후속 milestone 후보에서 제외한다.

이 프로젝트는 read-only tool에 집중한다. `rename_symbol`, `code_actions`,
`apply_code_action`, `format_document` 같은 write/refactoring tool은 계획하지
않는다. MCP 클라이언트와 agent가 이미 파일 편집 권한을 갖고 있으므로,
이 서버는 변경 적용자가 아니라 Roslyn 기반 판단 근거를 제공하는 역할로
경계를 둔다.
...
- diagnostics notification offload - 완료
  ...
  - overflow 정책은 `drop_newest_when_full`이고 pending/processed/dropped/stale 통계를 `get_workspace_status`에 노출한다. Queue가 full이면 새 notification을 drop하고 기존 pending notification을 유지한다.
...
Gemini CLI 실사용 피드백 기준으로, 에이전트가 Roslyn 위치 정보를 받은 뒤 다시 파일을 읽어야 하는 왕복 비용과 interface/hierarchy 탐색 비용을 줄이는 read-only 도구를 추가한다.
...
- `find_symbols` kind/match/path prefix filtering - 완료
  - 입력: `query`, optional `maxResults`, optional `kindFilter`, optional `matchMode`, optional `includePathPrefixes`
  - 기존 LSP `workspace/symbol` 결과를 `class`, `interface`, `method`, `property`, `field` 등 LSP `SymbolKind`, `default`, `exact`, `prefix`, `contains` symbol name match mode, root-relative path prefix 기준으로 제한한다.
  - Roslyn LS 검색 비용 절감보다는 MCP 반환 결과의 noise 감소와 agent 후속 탐색 효율 개선을 목표로 한다.
  - metadata는 모든 필터 후 mappable 결과 기준의 `totalKnown`, `returned`, `truncated`와 필터 전 mappable 결과 수인 `totalUnfilteredKnown`을 구분한다.
- `peek_references` - 완료
  - 입력: `file`, `line`, `column`, optional `includeDeclaration`, optional `maxResults`, optional `contextLines`
  - LSP `textDocument/references` 결과와 root-relative file, 1-based range, 제한된 source snippet을 함께 반환한다.
  - snippet은 `peek_definition`과 같은 path guard, `MaxDocumentBytes`, context line, character cap 제한을 따른다.
  - agent가 reference 사용 맥락을 확인하기 위해 각 파일을 다시 여는 왕복을 줄이는 것을 목표로 한다.
- `get_call_hierarchy` - 완료
  - 입력: `file`, `line`, `column`, `direction`, optional `maxResults`
  - LSP `textDocument/prepareCallHierarchy`, `callHierarchy/incomingCalls`, `callHierarchy/outgoingCalls`를 사용한다.
  - incoming은 특정 callable을 호출하는 위치를, outgoing은 특정 callable 내부에서 호출하는 대상을 반환한다.
  - 직접 depth-1 호출 관계만 반환하고 recursive `maxDepth`는 제공하지 않는다.
  - 대형 solution에서 호출 그래프 폭주를 막기 위해 expensive request limit, edge result cap, per-edge call-site cap을 적용한다.
  - `get_type_hierarchy`와 겹치지 않는다. `get_type_hierarchy`는 타입의 상속/파생 관계를 다루고, `get_call_hierarchy`는 메서드/생성자/속성 접근자 같은 callable의 런타임 호출 흐름에 가까운 정적 호출 관계를 다룬다.
- `get_type_hierarchy` - 완료
  - 입력: `file`, `line`, `column`, `direction`, optional `maxDepth`, optional `maxResults`
  - LSP `textDocument/prepareTypeHierarchy`, `typeHierarchy/supertypes`, `typeHierarchy/subtypes`를 사용한다.
  - 대형 solution에서 결과 폭주를 막기 위해 expensive request limit과 depth/result cap을 적용한다.
  - 호출자/피호출자 분석은 `get_call_hierarchy`에서 처리하므로 이 tool은 base type, derived type, interface implementation 같은 타입 구조 탐색에 집중한다.
이 milestone은 모두 read-only다. 파일 수정, rename apply, code action apply는 포함하지 않는다.
`get_completions`는 LSP `textDocument/completion`을 읽기만 하더라도 IDE 자동완성 기반의 write-adjacent 도구로 분류해 이 milestone에서 제외한다.

### 변경 작업 도구 - 비목표

rename, code action, formatting, apply 계열 tool은 이 프로젝트의 roadmap에
두지 않는다. 향후 추가되는 기능도 agent의 읽기 생산성, 대형 repository
readiness, 상태 관측성, best-effort metadata 품질을 높이는 방향으로 제한한다.
`get_completions`도 직접 edit을 적용하지는 않지만 코드 작성 후보를 제공하는
성격이 강하므로 현재 roadmap에는 포함하지 않는다.
