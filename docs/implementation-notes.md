...
- git scan은 전체 scan budget을 독점하지 않고 bounded portion만 사용한다. git scan이 실패하거나 timeout되면 남은 budget으로 filesystem fallback을 시도한다.
- `scan_timeout`은 non-cooperative git scanner가 budget을 넘기거나 filesystem fallback이 남은 budget 안에 끝나지 못할 때 발생한다.
