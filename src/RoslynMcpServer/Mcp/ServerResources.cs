...
public static class ServerResources
{
    ...
    public static string Guide() => """
        ...
        Start with list_workspaces and get_workspace_status, then load a solution or project when needed. Prefer Roslyn semantic tools for navigation. Use document_symbols with kindFilter, query, and maxResults when a large file outline should focus on specific symbol kinds or names while keeping ancestor context. Without query, document_symbols returns symbols in source order; with query, it ranks exact, case-insensitive exact, prefix, and contains matches first. Use find_symbols when you know a symbol name but not its file; default matching keeps Roslyn LS fuzzy candidates but ranks exact, prefix, and contains name matches first. Pass matchMode: "exact", "prefix", or "contains" when Roslyn LS default matching is too broad. Use includePathPrefixes when a large repository search should stay inside known root-relative directories; it is supported by symbol, reference, implementation, call hierarchy, and type hierarchy result tools. Use get_call_hierarchy on callable positions when you need direct depth-1 incoming callers, outgoing callees, or both. For method-only call graph reviews, pass kindFilter: ["method"]; include "constructor" or "property" only when those edges are relevant.
