...
public static class ServerResources
{
    ...
    public static string Guide() => """
        ...
        Start with list_workspaces and get_workspace_status, then load a solution or project when needed. Prefer Roslyn semantic tools for navigation. Use find_symbols when you know a symbol name but not its file; pass matchMode: "exact", "prefix", or "contains" when Roslyn LS default matching is too broad, and includePathPrefixes when a large repository search should stay inside known root-relative directories. Use includePathPrefixes with find_references or peek_references when you need only reference locations under specific root-relative directories, such as src/ or tests/. Use get_call_hierarchy on callable positions when you need direct depth-1 incoming callers, outgoing callees, or both. For method-only call graph reviews, pass kindFilter: ["method"]; include "constructor" or "property" only when those edges are relevant.
    ...
    public static string Capabilities() => """
        This server exposes read-only Roslyn context: workspace discovery, symbols, hover, definitions, references, implementations, call hierarchy, snippets, and diagnostics. Symbol and reference searches can be narrowed with MCP-side root-relative path prefix filters where supported.
